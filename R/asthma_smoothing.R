setwd("C:/Users/TMPACGAG/OneDrive - Birmingham City Council/Documents/R projects/PHM/BCC_UOB_air_quality")
...
########################################################################################################################
#asthma data
asthma_df = read_excel("~/R projects/PHM/BCC_UOB_air_quality/data/air-quality-related-admissions-by-lsoa-2025.xlsx", 
                                                          sheet = "Asthma")
#lsoa shape
...
#filter for adults
asthma_adults = asthma_df %>% 
  filter(LSOA_CODE %in% lsoa21_map$LSOA21CD) %>% 
  right_join(bham_lsoa_pop, by = c("LSOA_CODE"="LSOA21CD")) %>% 
  mutate(Children_Admissions = ifelse(is.na(Children_Admissions),0,Children_Admissions),
         Adult_Admissions = ifelse(is.na(Adult_Admissions),0,Adult_Admissions)) %>% 
  select(-Children_Admissions)
  

#filter for young 
asthma_young = asthma_df %>% 
  filter(LSOA_CODE %in% lsoa21_map$LSOA21CD) %>% 
  right_join(bham_lsoa_pop, by = c("LSOA_CODE"="LSOA21CD")) %>% 
  mutate(Children_Admissions = ifelse(is.na(Children_Admissions),0,Children_Admissions),
         Adult_Admissions = ifelse(is.na(Adult_Admissions),0,Adult_Admissions)) %>% 
  select(-Adult_Admissions)
########################################################################################################################
##################################################
...
formula = Adult_Admissions ~ 1 + f(new_id,
                        model = "bym2",
                        graph = g,
                        hyper = list(
...
data_smoothed_adults = asthma_adults %>%
  cbind(
...
formula = Children_Admissions ~ 1 + f(new_id,
                                   model = "bym2",
                                   graph = g,
                                   hyper = list(
                                     # PC prior for precision (overall variability / smoothing strength)
...
data_smoothed_young = asthma_young %>%
  cbind(
    fitmod$summary.fitted.values[, c("mean", "0.025quant", "0.5quant", "0.975quant")]
  ) %>% 
  as.data.frame() %>% 
...
write.xlsx(
  list(
    Asthma_children = data_smoothed_young,
    Asthma_Adults   = data_smoothed_adults
  ),
  file = "output/air_quality_spatial_Smoothing.xlsx",
  rowNames = FALSE
)
